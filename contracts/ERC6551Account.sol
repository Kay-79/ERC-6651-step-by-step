...
pragma solidity ^0.8.0 ^0.8.1 ^0.8.12 ^0.8.13 ^0.8.2 ^0.8.4 ^0.8.9;
...
// lib/erc6551/src/interfaces/IERC6551Account.sol
...
// lib/erc6551/src/interfaces/IERC6551Executable.sol
...
// lib/erc6551/src/lib/ERC6551BytecodeLib.sol
...
// src/interfaces/IAccountGuardian.sol
...
// src/interfaces/ISandboxExecutor.sol
    ...
    function sandbox(address owner) internal view returns (address) {
        return
            Create2.computeAddress(
                keccak256("org.tokenbound.sandbox"),
                keccak256(bytecode(owner))
            );
    }
...
// lib/erc6551/src/lib/ERC6551AccountLib.sol
...
library LibExecutor {
    ...
    function _execute(
        ...
        if (operation == OP_CREATE)
            return abi.encodePacked(_create(value, data));
...
abstract contract Overridable {
    ...
    function _handleOverride() internal virtual {
        ...
        if (implementation != address(0)) {
            address sandbox = LibSandbox.sandbox(address(this));
            (bool success, bytes memory result) = sandbox.call(
                abi.encodePacked(implementation, msg.data, msg.sender)
            );
            assembly {
                if iszero(success) {
                    revert(add(result, 32), mload(result))
...
abstract contract SandboxExecutor is ISandboxExecutor {
    ...
    function _requireFromSandbox() internal view {
        if (msg.sender != LibSandbox.sandbox(address(this)))
...
abstract contract ERC4337Account is BaseAccount {
    ...
    function _validateSignature(
        ...
        if (
            _isValidSignature(
                _getUserOpSignatureHash(userOp, userOpHash),
                userOp.signature
            )
        ) {
            return 0;
     ...
     */
    function _getUserOpSignatureHash(
